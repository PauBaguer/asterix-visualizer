...
export function probIdentification() {
    ...
    const sec = 5;
    workerData.decodedMsg.forEach((msg: Cat10 | Cat21) => {
...
export function computeAccuracy() {
    ...
    maneouvering.sort(function (a, b) { return b - a }).slice(Math.round(maneouvering.length * 0.1));
    ...
    const maneouvering_std = Math.sqrt(maneouvering.reduce((partialSum, a) => partialSum + Math.pow(a - maneouvering_average, 2), 0) / maneouvering.length);

    let apron = apronT1.concat(apronT2);
    apron.sort(function (a, b) { return b - a }).slice(Math.round(apron.length * 0.1));
    ...
    const stand_std = Math.sqrt(stand.reduce((partialSum, a) => partialSum + Math.pow(a - stand_average, 2), 0) / stand.length);
