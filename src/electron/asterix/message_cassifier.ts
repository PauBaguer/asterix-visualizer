...
export async function decodeClass21Messages(buffer: Buffer, cat21msg: Buffer[]) {
  var vec21: Cat21[] = [];
  let b = 0;
  //var test2 = 139747;
  await cat21msg.forEach(async (msg) => {
        ...
        if (fspec[23] === "1") {
          ...
          if (fspec[31] === "1") {
            ...
            if (fspec[34] === "1") {
              ...
              tasks.push(
                decod_msg.set_met_information(msg.slice(offset + 1, offset + fields.length * 2 + len), fields)
              );
